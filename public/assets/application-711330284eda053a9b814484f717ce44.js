...
(function() {
  ...
  $(document).ready(function() {
    ...
    $('.form-control').bind("onafteraddClass", function(e) {
      ...
      if ($(this).hasClass('parsley-error')) {
        ...
        return popup.popover("show");
      } else {
        popup.removeClass("error");
        return popup.popover("destroy");
      }
    });
    $('.destination').on("submit", function() {
      if ($(this).parsley('validate')) {
        ...
        return $('#origin').animate({
          left: "50%"
        }, 800);
      }
    });
    return $('.origin').on("submit", function() {
      if ($(this).parsley('validate')) {
        window.origin = $('.origin .address').val() + ',' + $('.origin .zip').val();
        $.getJSON("http://maps.googleapis.com/maps/api/directions/json?origin=" + window.origin + "&destination=" + window.destination + "&sensor=false", function(data) {
          return $('.drive_time').val(data.routes[0].legs[0].duration.value);
        });
        $('#origin').animate({
          left: "-50%"
        }, 800);
